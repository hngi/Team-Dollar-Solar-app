	...
	<div id = "app">
		...
		<div>
			...
			<div class="name-input">
					...
					<input id ="file_content" name = "file" type="file" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" required/>
				</div>
				<div>
					<input type="submit" name = "mass_calc_button" value = "Calculate and Download">
				</div>
				<div>
					<a href = "<?php echo $base_url.'test__document.xlsx'; ?>">Download Sample</a>
				</div>
			</form>
			<?php
			 }
				...
				if(in_array($_FILES["file_content"]["type"],$allowedFileType)){
						//exit(getcwd());
						$targetPath = '/uploads/'.$_FILES['file_content']['name'];

						//$targetPath = getcwd().'uploads/'.$_FILES['file_content']['name'];
		        move_uploaded_file($_FILES['file_content']['name'], $targetPath);
						$inputFileType = 'Xlsx';

						//$inputFileName = getcwd().'uploads/test_document.xlsx';
						$inputFileName = '/uploads/'.$_FILES['file_content']['name'];
